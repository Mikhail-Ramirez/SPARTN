...
def localize_source(time_lags, reordered_mics):
    ...
    # Convert time lags to distances (r = time_lag * speed_of_sound)
    r1 = time_lags.get(reordered_mics[1], 0) * SPEED_OF_SOUND
    r2 = time_lags.get(reordered_mics[2], 0) * SPEED_OF_SOUND
    r3 = 0  # Reference mic has no distance to itself
    ...
    # Create the system of equations for trilateration
    A = np.array([2 * (tower2 - tower1), 2 * (tower3 - tower1)]).T
    b = np.array([r1**2 - r2**2 - np.dot(tower1, tower1) + np.dot(tower2, tower2),
                  r1**2 - r3**2 - np.dot(tower1, tower1) + np.dot(tower3, tower3)])

    logging.info(f"[Trilateration] A_matrix: {A}")
    logging.info(f"[Trilateration] b_vector: {b}")
    
    try:
        estimated_position = np.linalg.solve(A, b)  # Solve for the position
        logging.info(f"[Trilateration] Estimated position: x = {estimated_position[0]:.3f}, y = {estimated_position[1]:.3f}")
    except np.linalg.LinAlgError:
        logging.error("[Trilateration] Error: Singular matrix during trilateration.")
