...
#define NUM_SECONDS 10            // Recording duration set to 10 seconds
#define NUM_CHANNELS 1            // Mono input
#define SAMPLE_TYPE paFloat32
typedef float SAMPLE;

// Function to list USB input devices and select the first two
bool getDualUSBInputDevices(int &deviceIndex1, int &deviceIndex2) {
    ...
    for (int i = 0; i < numDevices; i++) {
        ...
        if (deviceInfo->maxInputChannels > 0 && std::string(deviceInfo->name).find("USB") != std::string::npos) {
            if (deviceIndex1 == -1) {
                deviceIndex1 = i;
                std::cout << "Selected USB Device 1: " << deviceInfo->name << std::endl;
            }
            else if (deviceIndex2 == -1) {
                deviceIndex2 = i;
                std::cout << "Selected USB Device 2: " << deviceInfo->name << std::endl;
                break; // Found two devices
            }
        }
    }

    if (deviceIndex1 == -1 || deviceIndex2 == -1) {
        std::cerr << "Error: Expected 2 USB input devices but found "
                  << ((deviceIndex1 != -1) ? "1." : "0.") << std::endl;
        return false;
    }
...
// Structure to hold audio data for each microphone
struct AudioData {
...
bool writeWavFile(const std::string& filename, const std::vector<SAMPLE>& data, int sampleRate, int channels) {
    ...
    if (!outfile) {
        std::cerr << "Error opening output file " << filename << ": " << sf_strerror(NULL) << std::endl;
        return false;
    }
    ...
    if (count != static_cast<sf_count_t>(intData.size())) {
        std::cerr << "Error writing to WAV file " << filename << "." << std::endl;
        sf_close(outfile);
        return false;
    }
...
void plotAudioData(const std::vector<SAMPLE>& data, const std::string& title) {
    // Window dimensions
    const int WIDTH = 800;
    ...
    // Draw title
    if (font) {
        int titleWidth = XTextWidth(font, title.c_str(), title.size());
        int titleX = (WIDTH - titleWidth) / 2;
        int titleY = MARGIN_TOP / 2 + 5;
        XDrawString(display, win, gc, titleX, titleY, title.c_str(), title.size());
    }
    ...
    if (font) {
        ...
        int yLabelY = MARGIN_TOP + (HEIGHT - MARGIN_TOP - MARGIN_BOTTOM) / 2 - (yLabel.size() * 6) / 2;
    ...
    // Inform the user
    std::cout << "Plot \"" << title << "\" displayed. Close the window or press a key to exit." << std::endl;
...
int main() {
    ...
    if (!getDualUSBInputDevices(deviceIndex1, deviceIndex2)) {
        Pa_Terminate();
        return 1;
    }

    // Set up input parameters for Mic 1
    PaStreamParameters inputParameters1;
    inputParameters1.device = deviceIndex1;
    inputParameters1.channelCount = NUM_CHANNELS;
    inputParameters1.sampleFormat = SAMPLE_TYPE;
    inputParameters1.suggestedLatency = Pa_GetDeviceInfo(deviceIndex1)->defaultLowInputLatency;
    inputParameters1.hostApiSpecificStreamInfo = NULL;

    // Set up input parameters for Mic 2
    PaStreamParameters inputParameters2;
    inputParameters2.device = deviceIndex2;
    inputParameters2.channelCount = NUM_CHANNELS;
    inputParameters2.sampleFormat = SAMPLE_TYPE;
    inputParameters2.suggestedLatency = Pa_GetDeviceInfo(deviceIndex2)->defaultLowInputLatency;
    inputParameters2.hostApiSpecificStreamInfo = NULL;

    // Initialize AudioData structures for both microphones
    AudioData data1;
    AudioData data2;
    ...
    err = Pa_OpenStream(&stream1,
                        ...
                        &data1);
    if (err != paNoError) {
        std::cerr << "Error opening stream for Mic 1: " << Pa_GetErrorText(err) << std::endl;
        Pa_Terminate();
        return 1;
    }
    ...
    err = Pa_OpenStream(&stream2,
                        ...
                        &data2);
    if (err != paNoError) {
        std::cerr << "Error opening stream for Mic 2: " << Pa_GetErrorText(err) << std::endl;
    ...
    if (err != paNoError) {
        std::cerr << "Error starting stream for Mic 1: " << Pa_GetErrorText(err) << std::endl;
        Pa_CloseStream(stream1);
    ...
    if (err != paNoError) {
        std::cerr << "Error starting stream for Mic 2: " << Pa_GetErrorText(err) << std::endl;
        Pa_CloseStream(stream1);
        Pa_CloseStream(stream2);
        Pa_Terminate();
        return 1;
    ...
    std::cout << "Recording started for " << NUM_SECONDS << " seconds. Please speak into both microphones..." << std::endl;
    ...
    if (err != paNoError) {
        std::cerr << "Error stopping stream for Mic 1: " << Pa_GetErrorText(err) << std::endl;
    }

    err = Pa_StopStream(stream2);
    if (err != paNoError) {
        std::cerr << "Error stopping stream for Mic 2: " << Pa_GetErrorText(err) << std::endl;
    }
    ...
    // Save to WAV files
    std::string wavFilename1 = "camera1_audio.wav";
    std::string wavFilename2 = "camera2_audio.wav";

    if (writeWavFile(wavFilename1, data1.samples, SAMPLE_RATE, NUM_CHANNELS)) {
        std::cout << "Audio data from Mic 1 saved to " << wavFilename1 << std::endl;
    } else {
        std::cerr << "Failed to save audio data from Mic 1 to WAV file." << std::endl;
    }

    if (writeWavFile(wavFilename2, data2.samples, SAMPLE_RATE, NUM_CHANNELS)) {
        std::cout << "Audio data from Mic 2 saved to " << wavFilename2 << std::endl;
    } else {
        std::cerr << "Failed to save audio data from Mic 2 to WAV file." << std::endl;
    }

    // Plot the audio data for Mic 1
    std::thread plotThread1(plotAudioData, std::cref(data1.samples), "Mic 1 - Camera 1");

    // Plot the audio data for Mic 2
    std::thread plotThread2(plotAudioData, std::cref(data2.samples), "Mic 2 - Camera 2");

    // Wait for both plotting threads to finish
    plotThread1.join();
    plotThread2.join();
